...
SELECT YEAR(date_stolen), COUNT(vehicle_id) AS num_vehicles FROM stolen_vehicles
GROUP BY YEAR(date_stolen);

-- 2. Find the number of vehicles stolen each month. 
SELECT MONTH(date_stolen), COUNT(vehicle_id) AS num_vehicles FROM stolen_vehicles
GROUP BY MONTH(date_stolen);

SELECT YEAR(date_stolen), MONTH(date_stolen), COUNT(vehicle_id) AS num_vehicles FROM stolen_vehicles
GROUP BY YEAR(date_stolen), MONTH(date_stolen)
ORDER BY YEAR(date_stolen), MONTH(date_stolen);

-- Checking why the number of stolen vehicles seems to decrese in month 4
SELECT YEAR(date_stolen), MONTH(date_stolen), DAY(date_stolen), COUNT(vehicle_id) AS num_vehicles FROM stolen_vehicles
...
SELECT DAYOFWEEK(date_stolen) AS dow, COUNT(vehicle_id) AS num_vehicles FROM stolen_vehicles
...
-- For this step, I imported the data as a .csv file to read it into Excel and create a visualization. 
...
SELECT vehicle_type, COUNT(vehicle_id) AS num_vehicles FROM stolen_vehicles
GROUP BY vehicle_type
ORDER BY num_vehicles DESC
LIMIT 5; 

-- Least commonly stolen vehicles: 
SELECT vehicle_type, COUNT(vehicle_id) AS num_vehicles FROM stolen_vehicles
GROUP BY vehicle_type
...
SELECT vehicle_type, AVG(YEAR(date_stolen) - model_year)AS avg_age FROM stolen_vehicles 
GROUP BY vehicle_type
ORDER BY avg_age DESC;

-- 3. For each vehicle type, find the percentage of vehicles stolen that are luxury versus standard.
SELECT * FROM stolen_vehicles;
...
FROM stolen_vehicles sv LEFT JOIN make_details md ON sv.make_id = md.make_id)
SELECT vehicle_type, SUM(LUXURY)/SUM(all_cars) * 100 AS pct_lux
FROM lux_standard
GROUP BY vehicle_type
ORDER BY pct_lux DESC;

-- 4. Creating a table where the rows represent the top 10 vehicle types, the columns represent the top 7 vehicle colors (plus 1 column for all other colors) and the values are the number of vehicles stolen. 
SELECT * FROM stolen_vehicles;

# color, num_vehicles (most common stolen colors)
...
SUM(CASE WHEN color = 'White' THEN 1 ELSE 0 END)AS white,
   ...
   SUM(CASE WHEN color IN ('Cold', 'Brown', 'Yellow', 'Orange', 'Purple', 'Cream', 'Pink') THEN 1 ELSE 0 END)AS other
FROM stolen_vehicles
GROUP BY vehicle_type
ORDER BY num_vehicles DESC
LIMIT 10;
...
-- For this step, I exported the data from the last query into Excel to create a heat map. 
...
FROM stolen_vehicles sv LEFT JOIN locations l ON sv.location_id = l.location_id
GROUP BY region;
...
-- 3. Do the type of vehicles stolen in the three most dense regions differ from the three least dense regions?
SELECT l.region, COUNT(sv.vehicle_id) AS num_vehicles, l.population, l.density
FROM stolen_vehicles sv LEFT JOIN locations l ON sv.location_id = l.location_id
GROUP BY l.region, l.population, l.density
ORDER BY l.density DESC;

-- MOST COMMON VEHICLES STOLEN IN MOST DENSE REGIONS: 
...
-- MOST COMMON VEHICLES STOLEN IN LESS DENSE REGIONS: 
SELECT sv.vehicle_type, COUNT(sv.vehicle_id) AS num_vehicles
FROM stolen_vehicles sv LEFT JOIN locations l ON sv.location_id = l.location_id 
...
-- UNIFYING BOTH TABLES TO SEE RESULTS SIDE BY SIDE: 
...
-- Created visualization in Excel using data from question 2 (objecttive 3)

-- 5. Data visualization: create a map
-- Done in Excel with data from question 2 (objecttive 3)
